...
class decode_videos_to_clips(wds.PipelineStage):
    ...
    def run(self, src):
        for path in src:
            ...
            stop_frames = ano["end_frame"].values
            ...
            for i, (sf, ef) in enumerate(zip(start_frames, stop_frames)):
                ...
                labels_class = int(ano["action_class"].values[i])
                ...
                yield dict(
                    ...
                    action=labels_class,
                    path= str(ano["video_id"].values[i]),
                    start_time= ano["start_time"].values[i],
                    end_time=ano["end_time"].values[i],
...
def get_video_wds_dataset(
    ...
    pipeline = [
        ...
        wds.to_tuple("video", "action","path","start_time","end_time","anticipation_time"),
...
def filter_annotations(
    ...
    tactions = set([n for  n in tdf["action_class"].values])
    ...
    def build_annotations(df):
        ...
        for uv in unique_videos:
            fpath = df.loc[df["video_id"] == uv, "local_path"].iloc[0]
...
def make_webvid(
    ...
    **kwargs,
    ...
    #print("paths of the first four")
    #for p in paths[:4]:

        #print(p)
        #print(annotations[p.split("/")[-1].split(".")[0]])
